...
s32 DVD_GcodeLowWriteBuffer(void *buf,dvdcallbacklow cb);
s32 DVD_GcodeLowWrite(u32 offset,dvdcallbacklow cb);
...
static void __dvd_statebusycb(s32 result)
{
	...
	if(result&0x0001) {
		...
		if(__dvd_currcmd==0x0015) {
			if(!__dvd_executing->currtxsize) {
				__dvd_executing->currtxsize = 1;
				DVD_GcodeLowWrite(__dvd_executing->offset+__dvd_executing->txdsize,__dvd_statebusycb);
				return;
			}
			if(_diReg[8]) {
				block = __dvd_executing;
...
static void __dvd_synccb(s32 result,dvdcmdblk *block)
{
...
static void __dvd_statebusy(dvdcmdblk *block)
{
	...
	switch(block->cmd) {
		...
		case 21:
			block->currtxsize = 0;
			DVD_GcodeLowWriteBuffer(block->buf+(DVD_GCODE_BLKSIZE*block->txdsize),__dvd_statebusycb);
			return;
		default:
			return;
	}
...
{
#ifdef _DVD_DEBUG
	printf("DVD_GcodeLowWriteBuffer(%p)\n",buf);
#endif
	struct timespec tb;
...
#endif
	ret = DVD_InquiryAsync(block,info,__dvd_synccb);
...
#endif
	ret = DVD_StopStreamAtEndAsync(block,__dvd_synccb);
	if(!ret) return DVD_ERROR_FATAL;
...
#endif
	ret = DVD_StopMotorAsync(block,__dvd_synccb);
	if(!ret) return DVD_ERROR_FATAL;
...
#endif
	ret = DVD_SpinUpDriveAsync(block,__dvd_synccb);
	if(!ret) return DVD_ERROR_FATAL;
...
#endif
	ret = DVD_ControlDriveAsync(block,cmd,__dvd_synccb);
	if(!ret) return DVD_ERROR_FATAL;
...
#endif
	ret = DVD_SetGCMOffsetAsync(block,offset,__dvd_synccb);
	if(!ret) return DVD_ERROR_FATAL;
...
#endif
	ret = DVD_GcodeWriteAsync(block,buf,len,offset,__dvd_synccb);
	if(!ret) return DVD_ERROR_FATAL;
...
s32 DVD_Mount(void)
{
	...
	printf("DVD_Mount()\n");
#endif
	ret = DVD_MountAsync(&__dvd_block$15,__dvd_synccb);
	if(!ret) return DVD_ERROR_FATAL;
